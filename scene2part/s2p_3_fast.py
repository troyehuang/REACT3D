...
from utilities import visualize_masks
...
if __name__ == "__main__":
    ...
    # Precompute camera positions once (avoids re-parsing pose matrices per mesh)
    camera_positions = {}
    for stem in stem_list:
        if stem in pose_dict:
            c2w = np.array(pose_dict[stem]["aligned_pose"])
            camera_positions[stem] = c2w[:3, 3]
    ...
    for mesh_i, fname in tqdm(enumerate(mask_mesh_fnames), total=len(mask_mesh_fnames), desc="Processing meshes"):
        ...
        camera_relevance = get_camera_distances(stem_list, camera_positions, mesh_center)
        ...
        top_k_list = []
        good_iou_threshold = 0.8
        if_reach_good_iou = False
        ...
        for stem, distance in camera_relevance:
            ...
            # Get camera pose
            pose = pose_dict[stem]
            mvp = torch.from_numpy(np.array(pose["mvp"])).cuda().float()
            ...
            for mask_idx, mask_info in stem_masks:
                ...
                # visualize_masks(valid, binary_mask_resized)

                num_mask_pixel = np.count_nonzero(binary_mask)
                
                # if iou > 0.1:
                #     print(f"  Frame {stem}, Mask {mask_idx}: IoU={iou:.3f}, SqRatio={mask_info['sqratio']:.3f}")
                
                # Check for excellent match for early termination

                if len(top_k_list) < TOP_K and iou > 0.5:
                    top_k_list.append((iou, stem, mask_idx, binary_mask, num_mask_pixel))
                    top_k_list.sort(reverse=True, key=lambda x: x[0])
                    # if mesh_i == 141:
                    #     print("stem: ", stem)
                    #     print(mask_idx)
                    #     print(iou)
                    #     visualize_masks(valid, binary_mask_resized)
                    if len(top_k_list) == TOP_K and top_k_list[-1][0] >= good_iou_threshold:
                        if_reach_good_iou = True
                        print(f"  Reached good IoU threshold: for mesh {mesh_i}, frame {stem}, mask {mask_idx}")

                elif if_reach_good_iou and iou >= good_iou_threshold:
                    top_k_list.sort(reverse=True, key=lambda x: x[4])
                    if num_mask_pixel > top_k_list[-1][4]:
                        top_k_list[-1] = (iou, stem, mask_idx, binary_mask, num_mask_pixel)

                elif iou > 0.5 and iou > top_k_list[-1][0]:
                    top_k_list.sort(reverse=True, key=lambda x: x[0])
                    top_k_list[-1] = (iou, stem, mask_idx, binary_mask, num_mask_pixel)
                    top_k_list.sort(reverse=True, key=lambda x: x[0])

                    if top_k_list[-1][0] >= good_iou_threshold:
                        if_reach_good_iou = True
                        print(f"  Reached good IoU threshold: for mesh {mesh_i}, frame {stem}, mask {mask_idx}")
        ...
        if best_match is not None:
            top_k_list.insert(0, (best_match["iou"], best_match["stem"], best_match["mask_idx"], best_match["binary_mask"], best_match["num_mask_pixel"]))
            save_mesh_outputs(mesh_i, best_match, data_dir, save_dir)
            print(f"  Saved best match outputs (frame {best_match['stem']}, mask {best_match['mask_idx']})")
        # if potential_match is not None:
        #     top_k_list.insert(0, (potential_match["iou"], potential_match["stem"], potential_match["mask_idx"], potential_match["binary_mask"], potential_match["num_mask_pixel"]))
        ...
        #     top_k_list.insert(0, (potential_match_sec["iou"], potential_match_sec["stem"], potential_match_sec["mask_idx"], potential_match_sec["binary_mask"], potential_match_sec["num_mask_pixel"]))
        ...
        mesh_top_k_list[mesh_i] = top_k_list
    ...
    print(f"===============================================")
    for mesh_i, top_k in mesh_top_k_list.items():
        print(f"Mesh {mesh_i}: Top {TOP_K} matches:")
        for i, (iou, stem, mask_idx, _, num_mask_pixel) in enumerate(top_k):
            print(f"  {i+1}. Frame {stem}, Mask {mask_idx}: IoU={iou:.3f}, NumPixels={num_mask_pixel}")
    
    # Save top-k matches to file 
    with open(os.path.join(args.data_dir, 'perception', "top_k_matches.pkl"), 'wb') as f:
        pickle.dump(mesh_top_k_list, f)
